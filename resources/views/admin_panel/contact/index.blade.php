            ...
            <div class="card mb-6">
                <div class="card-body pt-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <h3>{{ __('messages.contact_list') }}</h3>
                        <div class="d-flex">
                            <a href="{{route('contact.create')}}" class="btn btn-primary my-2">{{ __('messages.create_contact') }}</a>
                        </div>
                    </div>
                    <div class="d-flex justify-content-between mb-3">
                        <div>
                            ...
                            <select id="entries" class="form-select form-select-sm d-inline-block w-auto">
                                <option value="{{route('contact.index', ['entries'=>5])}}">5</option>
                                <option value="{{route('contact.index', ['entries'=>25])}}">25</option>
                                <option value="{{route('contact.index', ['entries'=>50])}}">50</option>
                                <option value="{{route('contact.index', ['entries'=>100])}}">100</option>
                            </select>
                                    ...
                                    <div class="btn-group dropstart">
                                        ...
                                        <ul class="dropdown-menu me-3">
                                            <li><a class="dropdown-item"
                                                    href="{{route('contact.edit', $item->id)}}">{{ __('messages.edit') }}</a>
                                            </li>
                                            <li>
                                            ...
                                            <form hidden id="deleteButton_{{$item->id}}"
                                                action="{{route('contact.delete', $item->id)}}" method="POST">
                                                @csrf
                                                @method('DELETE')
...
<script>
    ...
    entriesSelection.value = "{{route('contact.index', ['entries'=>$numOfDataOnPage])}}";
