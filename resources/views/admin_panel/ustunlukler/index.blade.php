            ...
            <div class="card mb-6">
                <div class="card-body pt-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <h3>Ustunlukler</h3>
                        <div class="d-flex">
                            <a href="{{route('ustunlukler.create')}}" class="btn btn-primary my-2">Add Ustunlukler</a>
                        </div>
                    </div>
                    <div class="d-flex justify-content-between mb-3">
                        <div>
                            ...
                            <select id="entries" class="form-select form-select-sm d-inline-block w-auto">
                                <option value="{{route('ustunlukler.index', ['entries'=>5])}}">5</option>
                                <option value="{{route('ustunlukler.index', ['entries'=>25])}}">25</option>
                                <option value="{{route('ustunlukler.index', ['entries'=>50])}}">50</option>
                                <option value="{{route('ustunlukler.index', ['entries'=>100])}}">100</option>
                            </select>
                            <span class="">entries</span>
                        </div>
                        <div class="d-flex align-items-center">
                            <label for="search" class="me-2 mb-0">Search: </label>
                            <form action="{{route('ustunlukler.search')}}" method="GET">
                                @csrf
                                    ...
                                    <div class="btn-group dropstart">
                                        ...
                                        <ul class="dropdown-menu me-3">
                                            <li><a class="dropdown-item"
                                                    href="{{route('ustunlukler.edit', $item->id)}}">Edit</a>
                                            </li>
                                            <li>
                                            ...
                                            <form hidden id="deleteButton_{{$item->id}}"
                                                action="{{route('ustunlukler.delete', $item->id)}}" method="POST">
                                                @csrf
                                                @method('DELETE')
...
<script>
    ...
    entriesSelection.value = "{{route('ustunlukler.index', ['entries'=>$numOfDataOnPage])}}";
