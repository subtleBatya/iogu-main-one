            ...
            <div class="card mb-6">
                <div class="card-body pt-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <h3>{{ __('messages.notes') }}</h3>
                        <div class="d-flex">
                            <a href="{{route('notes.create')}}" class="btn btn-primary my-2">{{ __('messages.add_note') }}</a>
                        </div>
                    </div>
                    <div class="d-flex justify-content-between mb-3">
                        <div>
                            ...
                            <select id="entries" class="form-select form-select-sm d-inline-block w-auto" onchange="location = this.value;">
                                <option value="{{route('notes.index', ['entries'=>5])}}">5</option>
                                <option value="{{route('notes.index', ['entries'=>25])}}">25</option>
                                <option value="{{route('notes.index', ['entries'=>50])}}">50</option>
                                <option value="{{route('notes.index', ['entries'=>100])}}">100</option>
                            </select>
                                    ...
                                    <div class="btn-group dropstart">
                                        ...
                                        <ul class="dropdown-menu me-3">
                                            <li><a class="dropdown-item"
                                                    href="{{route('notes.edit', $item->id)}}">{{ __('messages.edit') }}</a>
                                            </li>
                                            <li>
                                            ...
                                            <form hidden id="deleteButton_{{$item->id}}"
                                                action="{{route('notes.delete', $item->id)}}" method="POST">
                                                @csrf
                                                @method('DELETE')
