            ...
            <div class="card mb-6">
                <div class="card-body pt-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <h3>Okuwbolumi</h3>
                        <div class="d-flex">
                            <a href="{{route('okuwbolumi.create')}}" class="btn btn-primary my-2">Add Okuwbolumi</a>
                        </div>
                    </div>
                    <div class="d-flex justify-content-between mb-3">
                        <div>
                            ...
                            <select id="entries" class="form-select form-select-sm d-inline-block w-auto">
                                <option value="{{route('okuwbolumi.index', ['entries'=>5])}}">5</option>
                                <option value="{{route('okuwbolumi.index', ['entries'=>25])}}">25</option>
                                <option value="{{route('okuwbolumi.index', ['entries'=>50])}}">50</option>
                                <option value="{{route('okuwbolumi.index', ['entries'=>100])}}">100</option>
                            </select>
                            <span class="">entries</span>
                        </div>
                        <div class="d-flex align-items-center">
                            <label for="search" class="me-2 mb-0">Search: </label>
                            <form action="{{route('okuwbolumi.search')}}" method="GET">
                                @csrf
                                    ...
                                    <div class="btn-group dropstart">
                                        ...
                                        <ul class="dropdown-menu me-3">
                                            <li><a class="dropdown-item"
                                                    href="{{route('okuwbolumi.edit', $item->id)}}">Edit</a>
                                            </li>
                                            <li>
                                            ...
                                            <form hidden id="deleteButton_{{$item->id}}"
                                                action="{{route('okuwbolumi.delete', $item->id)}}" method="POST">
                                                @csrf
                                                @method('DELETE')
...
<script>
    ...
    entriesSelection.value = "{{route('okuwbolumi.index', ['entries'=>$numOfDataOnPage])}}";
