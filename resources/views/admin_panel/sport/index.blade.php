            ...
            <div class="card mb-6">
                <div class="card-body pt-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <h3>{{ __('messages.sport') }}</h3>
                        <div class="d-flex">
                            <a href="{{route('sport.create')}}" class="btn btn-primary my-2">{{ __('messages.add_sport') }}</a>
                        </div>
                    </div>
                    <div class="d-flex justify-content-between mb-3">
                        <div>
                            ...
                            <select id="entries" class="form-select form-select-sm d-inline-block w-auto" onchange="location = this.value;">
                                <option value="{{route('sport.index', ['entries'=>5])}}">5</option>
                                <option value="{{route('sport.index', ['entries'=>25])}}">25</option>
                                <option value="{{route('sport.index', ['entries'=>50])}}">50</option>
                                <option value="{{route('sport.index', ['entries'=>100])}}">100</option>
                            </select>
                            <span class="">{{ __('messages.entries') }}</span>
                        </div>
                        <div class="d-flex align-items-center">
                            <label for="search" class="me-2 mb-0">{{ __('messages.search') }}: </label>
                            <form action="{{route('sport.search')}}" method="GET">
                                @csrf
                                    ...
                                    <div class="btn-group dropstart">
                                        ...
                                        <ul class="dropdown-menu me-3">
                                            <li><a class="dropdown-item"
                                                    href="{{route('sport.edit', $item->id)}}">{{ __('messages.edit') }}</a>
                                            </li>
                                            <li>
                                            ...
                                            <form hidden id="deleteButton_{{$item->id}}"
                                                action="{{route('sport.delete', $item->id)}}" method="POST">
                                                @csrf
                                                @method('DELETE')
